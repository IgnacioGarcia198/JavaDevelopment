...
class Baldosas {
    ...
    /**
     * Procedimiento iterativo que usa una lista enlazada para hacer un recorrido en 
     * amplitud del "árbol de cuadrantes", considerando cada cuadrante o subcuadrante 
     * creado como un nodo del árbol. En cada nodo Cuadrante, genera sus cuatro hijos, 
     * escribe el caracter actual en las coordenadas EsquinaCentral de tres de ellos 
     * que están vacíos, hasta haber escrito en los subcuadrantes de 2 x 2 casillas; 
     * el procedimiento escribeMinimoCuadrante se encarga de terminar de rellenar 
     * estos cuadrantes básicos.
     */       
    private void escribeCuadrantes() { // escribe los triminos en la tabla de 
        // caracteres de forma iterativa recorriendo una lista enlazada
        Lista listaArbol = new Lista(new Cuadrante(N));  // creamos una lista enlazada de objetos 
        // cuadrante que tiene como primer nodo el cuadrante raíz del árbol (equivalente a toda la tabla)
        NodoLista actual = listaArbol.Fin; // variable que recorre la lista; la 
        // igualamos a la cabeza de la lista, que inicialmente es también el fin.
        while (actual != null) {  // mientras no hayamos pasado el final de la lista
            if ((actual.Valor.lhs - actual.Valor.lhi) == 1)  // pasamos a la función 
            // sencilla que sólo escribe un trimino si el cuadrante actual sólo tiene 
            // 2 x 2 casillas
                escribeMinimoCuadrante(actual.Valor);
            else {
                for (int nhijo = 1; nhijo <= 4; nhijo ++) { // para los 4 hijos del 
                    // cuadrante actual (que está en la lista enlazada)
                    Cuadrante h = actual.Valor.Hijo(nhijo); 
                    // generamos el siguiente hijo
                    escribeLetra(h); 
                    // escribimos el caracter del trimino actual en él
                    listaArbol.insertar(h); 
                    // introducimos este hijo al final de la lista
                }
                avanzaCaracter(); // obtenemos el siguiente caracter a escribir 
            }
            actual = actual.Siguiente; // pasamos al siguiente cuadrante de la lista;
            // los que quedan detrás no tienen referencias y serán limpiados por el
    ...
    void baldosas() {
        if (N > 0) {
            Suelo = new char[N][N];
            for(int i = 0; i < N; i++) {  // inicializo la tabla llena de puntos, es 
                // decir símbolos de vacío
                for(int j = 0; j < N; j++) {
                    Suelo[i][j] = '.';
                }
            }
            ...
            if (N > 1) // la funcion solo es necesaria si hay alguna casilla más que 
            // rellenar a parte de '#' 
                escribeCuadrantes(); // invoco al algoritmo
            if (!trazar) // en caso de traza no escribimos la tabla al final para no
            // repetir la última escritura
                escribeSuelo();
        }
    }
